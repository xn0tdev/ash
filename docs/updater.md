...
1. **Check** — on startup (4s after launch) the frontend calls
   ...
   release tag (`vX.Y.Z`) against the build-time `main.Version` (semver,
   component-wise).
...
- A git tag `vX.Y.Z` triggers `.github/workflows/release.yml`, which builds
  `Ash.exe` with `-ldflags "-X main.Version=X.Y.Z -X main.Commit=<sha>"` and
...
- `main.Version` defaults to `"dev"` for local `wails build` (no ldflags), so
